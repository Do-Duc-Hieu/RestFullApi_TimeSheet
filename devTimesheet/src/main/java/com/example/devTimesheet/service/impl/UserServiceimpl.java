...
public class UserServiceimpl implements UserService {
    ...
    public UserRespon createUser(UserRequest request, MultipartFile avatar) throws IOException {
        ...
        user.setRole((Role) roleRepository
                .findRoleByNameRole(request.getRole().getNameRole())
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED)));
        user.setBranch((Branch) branchRepository
                .findBranchByNameBranch(request.getBranch().getNameBranch())
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED)));
        user.setWorkTime((WorkTime)
                workTimeRepository.findById(1).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED)));
        // ma hoa mat khau
    ...
    public UserRespon getUser(Integer id) {
        return userMapper.toUserRespon(
                userRepository.findById(id).orElseThrow(() -> new AppException(ErrorCode.USER_EXISTED)));
    }
    ...
    public List<UserRespon> findAllUser() {
        ...
        users.forEach(user -> userRespons.add(userMapper.toUserRespon(user)));
    ...
    public UserRespon updateUser(Integer idUser, UserUpdateRequest request) {
        User user = userRepository.findById(idUser).orElseThrow(() -> new AppException(ErrorCode.USER_EXISTED));
        user.setRole((Role) roleRepository
                .findRoleByNameRole(request.getRole().getNameRole())
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED)));
        user.setBranch((Branch) branchRepository
    ...
    public UserRespon getMyInfo() {
        ...
        User user =
                userRepository.findUserByUsername(name).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
        return userMapper.toUserRespon(user);
    }
}
