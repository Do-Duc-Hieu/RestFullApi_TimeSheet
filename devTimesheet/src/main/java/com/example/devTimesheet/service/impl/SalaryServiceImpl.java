...
import com.example.devTimesheet.mapper.PermissionMapper;
...
public class SalaryServiceImpl implements SalaryService {
    ...
    public List<SalaryRespon> timekeeping(YearMonth time){
        ...
        users.forEach(
                user -> {
                     List<Punishment> punishments = punishmentRepository.findByUserIdAndYearMonth(
                            user.getId(), time.getYear(), time.getMonthValue());
    ...
    public SalaryRespon getSalary (Integer id){
        return salaryMapper.toSalaryRespon(salaryRepository.findById(id)
    ...
    public List<SalaryRespon> findAllSalary(){
        ...
        salaries.forEach(
                salary -> salaryRespons.add(salaryMapper.toSalaryRespon(salary))
