...
public class RequestServiceImpl implements RequestService {
    ...
    public RequestRespon createRequest(RequestRequest requestRequest, MultipartFile image) throws IOException {
        ...
        if (request.getRequestType() instanceof RequestOff) {
            ...
            ((RequestOff) request.getRequestType())
                    .setLeaveType(leaveTypeRepository
                            .findLeaveTypeByNameType(((RequestOff) request.getRequestType())
                                    .getLeaveType()
                                    .getNameType())
        ...
        request.setUser((User) userRepository
                .findUserByUsername(userName)
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED)));
        request.setStatus(
                (Status) statusRepository.findById(1).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED)));
    ...
    public RequestRespon getRequest(Integer id) {
        Request request =
                requestRepository.findById(id).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
        log.info(request.getRequestType().getClass().getName());
        return requestMapper.toRequestRespon(request);
    }

    @Override
    public List<RequestRespon> getRequestByUserAndDateRangeExcludingWorkTime(LocalDate startDate, LocalDate endDate) {

        var context = SecurityContextHolder.getContext();
        String userName = context.getAuthentication().getName();
        List<RequestRespon> requestRespons = new ArrayList<>();
        List<Request> requests =
                requestRepository.findByUsernameAndDateRangeExcludingWorkTime(userName, startDate, endDate);
        requests.forEach(request -> requestRespons.add(requestMapper.toRequestRespon(request)));

        return requestRespons;
    }

    @Override
    public List<RequestRespon> getWorkTimeRequestsByUserAndDateRange(LocalDate startDate, LocalDate endDate) {

        var context = SecurityContextHolder.getContext();
        String userName = context.getAuthentication().getName();
        List<RequestRespon> requestRespons = new ArrayList<>();
        List<Request> requests =
                requestRepository.findAllWorkTimeRequestsByUserAndDateRange(userName, startDate, endDate);
        requests.forEach(request -> requestRespons.add(requestMapper.toRequestRespon(request)));
    ...
    @Override
    public List<RequestRespon> findAllWorkTimeRequestsByUserAndPending(String statusName, List<String> nameProjects) {

        var context = SecurityContextHolder.getContext();
        String userName = context.getAuthentication().getName();
        List<RequestRespon> requestRespons = new ArrayList<>();
        List<Request> requests =
                requestRepository.findAllWorkTimeRequestsByUserAndPending(userName, statusName, nameProjects);
        requests.forEach(request -> requestRespons.add(requestMapper.toRequestRespon(request)));
    ...
    public Request findById(Integer id) {
        return requestRepository.findById(id).orElseThrow(() -> new AppException(ErrorCode.USER_EXISTED));
    }
    ...
    public RequestRespon updateRequest(Integer idRequest, RequestRequest requestRequest) {
        ...
        Request request =
                requestRepository.findById(idRequest).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
    ...
    public RequestRespon browseRequest(Integer idRequest, RequestRequest requestRequest) {
        ...
        if (request.getRequestType() instanceof RequestWorkTime) {
            if (requestRequest.getIdStatus() == 2) {
                ...
                Status status =
                        statusRepository.findById(4).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
                List<Request> oldRequests = requestRepository.findAllWorkTimeRequestByUserAndStatusName(
                        request.getUser().getUsername(), nameStatus);
                ...
                User user = userRepository
                        .findUserByUsername(request.getUser().getUsername())
                        .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
                ...
                List<WorkTime> workTimes = workTimeRepository.findByTimes(
                        workTimeRequest.getMorningStartTime(),
                        workTimeRequest.getMorningEndTime(),
                        workTimeRequest.getAfternoonStartTime(),
                        workTimeRequest.getAfternoonEndTime());

                if (workTimes.size() < 1) {
                    WorkTimeRespon workTimeRespon = workTimeService.createWorkTime(workTimeRequest);
                    WorkTime workTime = workTimeRepository
                            .findById(workTimeRespon.getId())
                            .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
                    user.setWorkTime(workTime);
                } else {
                    WorkTime workTime = workTimeRepository
                            .findById(workTimes.get(0).getId())
                            .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
                    user.setWorkTime(workTime);
                }
        ...
        request.setStatus((Status) statusRepository
                .findById(requestRequest.getIdStatus())
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED)));
        request = requestRepository.save(request);
        log.info(requestMapper.toRequestRespon(request).getRequestTypeRespon().toString());
        return requestMapper.toRequestRespon(request);
    ...
    public void exportRequestToExcel(
        ...
        List<RequestRespon> requestRespons =
                searchRequestByProject(projectName, statusName, email, nameRequestType, startDate, endDate);
        ...
        try (Workbook workbook = new XSSFWorkbook();
            ...
            for (RequestRespon requestRespon : requestRespons) {
                ...
                row.createCell(4)
                        .setCellValue(requestRespon.getRequestTypeRespon().getType());
