...
public class TimeSheetServiceImpl implements TimeSheetService {
    ...
    public TimeSheetRespon createTimeSheet(TimeSheetRequest request){
        ...
        timeSheet.setUser((User) userRepository.findUserByUsername(userName)
                .orElseThrow(()-> new AppException(ErrorCode.USER_NOT_EXISTED)));
        timeSheet.setProject((Project) projectRepository.findProjectByNameProject(request.getNameProject())
                .orElseThrow(()-> new AppException(ErrorCode.USER_NOT_EXISTED)));
        timeSheet.setStatus((Status) statusRepository.findById(4)
                .orElseThrow(()-> new AppException(ErrorCode.USER_NOT_EXISTED)));
    ...
    public TimeSheetRespon getTimeSheet (Integer id){
        return timeSheetMapper.toTimeSheetRespon(timeSheetRepository.findById(id)
                .orElseThrow(()-> new AppException(ErrorCode.USER_NOT_EXISTED)));
    }
    ...
    public List<TimeSheetRespon> getTimeSheetByUserAndDate (LocalDate date){
        ...
        timeSheets.forEach(
                timeSheet -> timeSheetRespons.add(timeSheetMapper.toTimeSheetRespon(timeSheet))
    ...
    public List<TimeSheetRespon> getTimeSheetByUserAndDateRange (LocalDate startDate, LocalDate endDate){
        ...
        List<TimeSheetRespon> timeSheetRespons = new ArrayList<>();
        List<TimeSheet> timeSheets = timeSheetRepository
                .findByUsernameAndDateRange(userName, startDate, endDate);
        timeSheets.forEach(
                timeSheet -> timeSheetRespons.add(timeSheetMapper.toTimeSheetRespon(timeSheet))
    ...
    @Override
    public List<TimeSheetRespon> searchTimeSheet(String projectName, String userName,
    ...
    public TimeSheetRespon browseTimeSheet(Integer idTimeSheet, TimeSheetRequest request){
        TimeSheet timeSheet = timeSheetRepository.findById(idTimeSheet)
                .orElseThrow(()-> new AppException(ErrorCode.USER_NOT_EXISTED));
        timeSheet.setStatus((Status) statusRepository.findById(request.getIdStatus())
                .orElseThrow(()-> new AppException(ErrorCode.USER_NOT_EXISTED)));
        timeSheetMapper.updateTimeSheet(timeSheet, request);
        return timeSheetMapper.toTimeSheetRespon(timeSheetRepository.save(timeSheet));
    }
    ...
    public void exportTimeSheetsToExcel(
        ...
        List<TimeSheetRespon> timeSheets = searchTimeSheet(
                projectName, userName, branchName, statusName, type, startDate, endDate);
