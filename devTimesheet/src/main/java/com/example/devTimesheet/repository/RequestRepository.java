...
public interface RequestRepository extends JpaRepository<Request, Integer> {
    ...
    @Query("SELECT r FROM Request r " + "JOIN r.status s "
            + "WHERE r.user.username = :username "
            + "AND (:statusName IS NULL OR s.nameStatus = :statusName) "
            + "AND r.date = :date "
            + "AND TYPE(r.requestType) = RequestLast")
    List<Request> findAllLastRequestsByUserAndDate(
            @Param("username") String username, @Param("statusName") String statusName, @Param("date") LocalDate date);
    ...
    @Query("SELECT r FROM Request r " + "JOIN r.status s "
            ...
            + "AND r.date BETWEEN :startDate AND :endDate "
            + "AND (:statusName IS NULL OR s.nameStatus = :statusName) "
            + "AND TYPE(r.requestType) = RequestOff")
    List<Request> findAllOffRequestsByUserAndPending(
            @Param("username") String username,
    ...
    @Query("SELECT r FROM Request r " + "JOIN r.status s "
            ...
            + "AND TYPE(r.requestType) = RequestWorkTime")
    List<Request> findAllWorkTimeRequestByUserAndStatusName(
            @Param("username") String username, @Param("statusName") String statusName);
    ...
    @Query("SELECT r FROM Request r " + "JOIN r.status s "
            ...
            + "AND NOT EXISTS (SELECT 1 FROM RequestWorkTime rw WHERE rw.id = r.requestType.id)")
    List<Request> searchRequestsByProject(
    ...
    @Query("SELECT r FROM Request r " + "JOIN r.status s "
            ...
            + "AND (:endDate IS NULL OR r.date <= :endDate)"
            + "AND NOT EXISTS (SELECT 1 FROM RequestWorkTime rw WHERE rw.id = r.requestType.id)")
    List<Request> searchRequestsByBranch(
