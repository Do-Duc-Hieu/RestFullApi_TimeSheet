...
public interface TimeSheetRepository extends JpaRepository<TimeSheet, Integer> {
    ...
    List<TimeSheet> findByUsernameAndDate(@Param("username") String username, @Param("date") LocalDate date);
    ...
    @Query("SELECT ts FROM TimeSheet ts " + "JOIN ts.project p "
            + "JOIN ts.user u "
            + "JOIN u.branch b "
            + "JOIN ts.status s "
    ...
    List<TimeSheet> searchTimeSheets(
